...
// fmtPctAge — formats a price-change % only if the coin is old enough for
// the window to be meaningful. A 4-hour-old coin's "24h price change" is
// measured against a near-zero pre-existence price and reads as 1000%+,
// which Claude (correctly) interprets as "overextended." But the metric
// itself is structurally garbage for young coins. Returns "N/A (coin <Xh
// old)" instead so Claude doesn't penalize the coin for existing.
function fmtPctAge(value, ageHours, windowHours) {
  if (ageHours != null && ageHours < windowHours) {
    return `N/A (coin only ${ageHours.toFixed(1)}h old, no ${windowHours}h baseline)`;
  }
  return fmtPct(value);
}
