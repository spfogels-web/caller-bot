...
PRICE ACTION:
  5m:  ${fmtPct(candidate.priceChange5m)}  1h: ${fmtPctAge(candidate.priceChange1h, candidate.pairAgeHours, 1)}
  6h:  ${fmtPctAge(candidate.priceChange6h, candidate.pairAgeHours, 6)}  24h: ${fmtPctAge(candidate.priceChange24h, candidate.pairAgeHours, 24)}
...
// fmtPctAge — formats a price-change % with context about whether the
// window represents a true time-series comparison or just "price since
// launch." When a coin is younger than the window (e.g. a 30-min-old coin
// with a 1h price change, or a 4h-old coin with a 24h price change), the
// % isn't measuring the coin against a settled baseline — it's just
// measuring growth since the coin existed. That commonly reads as +500%
// to +5000% on a normal launch, which Claude (correctly) interprets as
// overextension if not given context.
//
// Keep the data — it's still useful trade info. Just annotate so the
// reader knows how to interpret it.
function fmtPctAge(value, ageHours, windowHours) {
  if (value == null) return 'MISSING';
  const formatted = fmtPct(value);
  if (ageHours != null && ageHours < windowHours) {
    return `${formatted} [since-launch, coin only ${ageHours.toFixed(1)}h old]`;
  }
  return formatted;
}
