...
function runMigrations() {
  const migrations = [
    ...
    // v10: Token metrics time-series — see DELTAS not just snapshots
    ...
    `CREATE INDEX IF NOT EXISTS idx_tmh_ca   ON token_metrics_history(contract_address, snapshot_at_ms DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_tmh_time ON token_metrics_history(snapshot_at_ms DESC)`,
    `DELETE FROM token_metrics_history WHERE created_at < datetime('now', '-6 hours')`,
